...
function ProductPage() {
  ...
  useEffect(() => {
    async function fetchProduct() {
      try {
        const response = await axios.get(`https://localhost:8080/api/Product/${productId}`);
          ...
          {loggedIn && userType > 0 ? 
            (product.isdiscounted === 0)? (
              ...
              <div className='offer-product'>
                <h5 className="price-s"><s>₹{product.productprice}</s></h5>
                <h4 className="discounted-price">₹{product.productprice - product.productprice*0.1}</h4>
              </div>
              )
          : (
