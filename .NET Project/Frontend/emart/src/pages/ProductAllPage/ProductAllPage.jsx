...
const ProductAllPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get(`https://localhost:8080/api/Product/subcategory/${subcategoryid}`);
                      ...
                      {loggedIn && userType === 1 ?
                        product.isdiscounted === 1 ? (
                          <div className='offer-product'>
                            <p className="price-s"><s>₹{product.productprice}</s></p>
                            <p className="discounted-price">₹{product.productprice - product.productprice * 0.1}</p>
                          </div>
                        ) : (
                          <div>
                            <p className='price-s'>₹{product.productprice}</p>
                            <div>
                              ...
                              />
                              <p className="price">₹{product.productprice - 100}{' + '}
